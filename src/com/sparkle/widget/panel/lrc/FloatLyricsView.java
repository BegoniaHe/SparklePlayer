 ...
 * 但是在翻译歌词和音译歌词上面，再用clip来实现是比较麻烦的，所以，这里使用image来实现 * 动感歌词效果；其实image内部也是使用Graphics2D和clip来实现动感歌词效果，并生成一个文本图片，
...
public class FloatLyricsView extends JPanel {
	...
	private void initSizeWord() {
		...
		mBaseFont = FontsUtil.getBaseFont(mFontSize);
	}
		...
		if (mLyricsLineNum % 2 == 0) {
			...
			if (mLyricsLineNum + 1 < mLyricsLineTreeMap.size()) {
				...
				drawBackground(g2d, lyricsRight, textRightX,
						getLineHeight(g2d) * 2);				int textHeight = getTextHeight(g2d);
				initPaintHLDEFColor(textRightX, getLineHeight(g2d) * 2,
						textHeight);
				g2d.setPaint(mPaint);
				drawStringWithFallback(g2d, lyricsRight, textRightX, getLineHeight(g2d) * 2);
			}
			...
			if (mLyricsLineNum + 1 != mLyricsLineTreeMap.size()) {
				...
				drawBackground(g2d, lyricsLeft, 10, getLineHeight(g2d));				int textHeight = getTextHeight(g2d);
				initPaintHLDEFColor(10, getLineHeight(g2d), textHeight);
				g2d.setPaint(mPaint);
				drawStringWithFallback(g2d, lyricsLeft, 10, getLineHeight(g2d));
			}
	...
	private void drawBackground(Graphics2D g2d, String string, float x, float y) {
		...
		drawStringWithFallback(g2d, string, x - 1, y);
		drawStringWithFallback(g2d, string, x + 1, y);
		drawStringWithFallback(g2d, string, x, y + 1);
		drawStringWithFallback(g2d, string, x, y - 1);
	}
	...
	private void drawStringWithFallback(Graphics2D g2d, String text, float x, float y) {
		...
		AttributedString attributedString = FontsUtil.createFallbackString(text, mFontSize);
	...
	private int getTextWidth(Graphics2D g2d, String text) {
		return DrawingUtil.getTextWidthWithFallback(g2d, text, mFontSize);
	}
