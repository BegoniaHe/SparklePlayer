...
import com.sparkle.tween.ValueAnimator;
import com.sparkle.tween.ValueAnimator.AnimationListener;
import com.sparkle.tween.ValueAnimator.AnimatorUpdateListener;
...
public class ManyLineLyricsView extends JPanel {
	...
	private void initSizeWord() {
		...
		mBaseFont = FontsUtil.getBaseFont(BaseData.lrcFontSize);
	}
	...
	private void drawTimeLine(Graphics2D g2d) {
		...
		drawStringWithFallback(g2d, timeString, 0, y - textHeight);
	...
	private void drawLineLrc(Graphics2D g2d, float centreY,
		...
		// 如果计算出的textX为负数，将textX置为0(实现：如果歌词宽大于view宽，则居左显示，否则居中显示)		textX = Math.max(textX, 10);
		g2d.setPaint(mPaint);
		// 画当前歌词
		drawStringWithFallback(g2d, text, textX, centreY);
	}
	/**
	 * 使用字体回退绘制字符串
	 * 
	 ...
	 * @param y
	 */
	private void drawStringWithFallback(Graphics2D g2d, String text, float x, float y) {
		AttributedString attrString = FontsUtil.createFallbackString(text, BaseData.lrcFontSize);
		g2d.drawString(attrString.getIterator(), x, y);
	}
	...
	private void drawDGBufferedImage(Graphics2D origG2d, float textX,
		...
		g2d.setPaint(mPaint);
		// 画当前歌词
		drawStringWithFallback(g2d, text, dgTextX, dgTextY);		// 这里不知为何还要减去fm.getDescent() + fm.getLeading() 绘画时才能把全文字绘画完整
	...
	private void drawDefText(Graphics2D g2d) {
		...
		int clipY = (int) (heightY - getRealTextHeight(g2d))
				+ getAdjustLrcHeight(g2d);
		g2d.setPaint(mPaint);
		drawStringWithFallback(g2d, mDefText, leftX, heightY);
	...
	private int getTextWidth(Graphics2D g2d, String text) {
		return DrawingUtil.getTextWidthWithFallback(g2d, text, BaseData.lrcFontSize);
	}
