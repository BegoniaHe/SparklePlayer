...
public class ListViewComItemPanel extends JPanel {
	...
	private void initDefComponent() {
		...
		songName = new FontFallbackLabel(songInfo.getDisplayName());
	...
	private void initDoubSelectedComponent() {
		...
		// Try to get album art, fallback to default icon
		ImageIcon albumIcon = getAlbumArtIcon();
		singerIconLabel = new JLabel(albumIcon);
		...
		this.add(songName);
		this.add(songSize);
		this.add(songProgress);		songListPanel.updateUI();
	}
	/**
	 * Get album art icon for the current song
	 * @return Circular album art icon or default icon if not available
	 */
	private ImageIcon getAlbumArtIcon() {
		try {
			// Check if we already have album art saved
			if (songInfo.getAlbumUrl() != null && !songInfo.getAlbumUrl().isEmpty()) {
				File albumFile = new File(songInfo.getAlbumUrl());
				if (albumFile.exists()) {
					BufferedImage albumImage = javax.imageio.ImageIO.read(albumFile);
					if (albumImage != null) {
						BufferedImage circularImage = ImageUtils.createCircularImage(albumImage, height * 3 / 4);
						return new ImageIcon(circularImage);
					}
				}
			}
			
			// Try to extract album art from the file
			byte[] albumArtData = MediaUtils.extractAlbumArt(songInfo.getFilePath());
			if (albumArtData != null) {
				return ImageUtils.createCircularImageIcon(albumArtData, height * 3 / 4);
			}
		} catch (Exception e) {
		...
		// Fallback to default circular icon
		return ImageUtils.getDefaultIcon(height * 3 / 4);
	}
